...
if not st.session_state.autenticado:
    ...
    if st.session_state.perfil == "juan":
        ...
        # ============================================================
        # PAINEL BRAYAN - VERSÃO SIMPLIFICADA
        # ============================================================
        ...
        # Saudação simples sem o contador de matérias
        st.markdown(f"""
        ...
        with tab_b1:
            ...
            if total_pautas == 0:
                ...
                st.error("🚨 **ATENÇÃO:** Você tem matérias **URGENTES** na fila!")
            else:
                st.info(f"📋 Você tem **{total_pautas}** postagens pendentes.")

            for p in pautas:
                pid, p_titulo, p_link, p_prioridade, p_hora, p_texto, p_status = p
                
                # Definição visual conforme o seu original
                ...
                if p_texto:
                    with st.expander("📄 VER TEXTO / RELEASE", expanded=False):
                        st.text_area("Copiar conteúdo:", value=p_texto, height=200, key=f"text_{pid}")
        ...
        with tab_b2:
            ...
            with st.form("form_agenda_brayan", clear_on_submit=True):
                st.markdown("##### ✍️ Novo Lembrete / Gravação")
            ...
            st.markdown("---")
            c.execute("SELECT id, data_ref, titulo, descricao, status, criado_por FROM agenda_itens WHERE (status = 'Pendente') OR (data_ref = ?) ORDER BY data_ref ASC", (hoje_str,))
            itens = c.fetchall()
