...
if not st.session_state.autenticado:
    ...
    if st.session_state.perfil == "juan":
        ...
        with tab2:
            with st.form("envio"):
                f_titulo = st.text_input("Título da Notícia"); f_link = st.text_input("Link (Opcional)"); f_obs = st.text_area("Instrução"); f_urgencia = st.select_slider("Prioridade", options=["Normal", "Programar", "URGENTE"])
        ...
        for pb in p_br:
            ...
            st.markdown(f'<div class="card-pauta {classe_cor}"><span class="tag-status {tag_cor}">{b_prio}</span> | 🕒 {b_hora}<br><p style="font-size: 1.4rem; font-weight: bold; margin-top:10px;">{b_tit}</p></div>', unsafe_allow_html=True)
            if b_obs: st.markdown(f'<div class="obs-box"><b>Instrução do Juan:</b><br>{b_obs}</div>', unsafe_allow_html=True)
            if b_link: st.link_button("🔗 ABRIR REFERÊNCIA", b_link, use_container_width=True)
            if st.button("✅ CONCLUÍDO / POSTADO", key=f"ok_{b_id}", use_container_width=True, type="primary"):
